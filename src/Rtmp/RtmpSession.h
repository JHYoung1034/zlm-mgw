...
/**
 * 此类用于描述rtmp服务会话
 * 1.通过继承Session类的onRecv，onError，onManager实现对会话的管理和数据接收
 * 2.通过继承RtmpProtocol的onSendRawData，onRtmpChunk两个接口函数，实现协议解析后处理发送和接收
 *   由RtmpProtocol类中协议处理时调用
 * 3.通过继承MediaSourceEvent类，重写某些接口，实现对媒体源事件的处理
 * 4.重要属性：_push_src, _play_src
*/
