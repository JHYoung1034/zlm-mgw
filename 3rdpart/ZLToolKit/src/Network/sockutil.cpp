...
static int bind_netif(int fd, const char *netif) {
    struct ifreq ifr;
    bzero(&ifr, sizeof(ifr));

    strncpy(ifr.ifr_name, netif, sizeof(ifr.ifr_name) - 1);
    if (0 > setsockopt(fd, SOL_SOCKET, SO_BINDTODEVICE, (char *)&ifr, sizeof(ifr))) {
        WarnL << "Bind socket to netif failed: " << netif;
        return -1;
    }
    return 0;
}

int SockUtil::connect(const char *host, uint16_t port, bool async, const char *netif, uint16_t local_port) {
    ...
    if ((netif && netif[0]) || local_port > 0) {
        if ((is_ipv4(netif) || is_ipv6(netif)) || local_port > 0) {
            if (bind_sock(sockfd, netif, local_port, addr.ss_family) == -1) {
                close(sockfd);
                return -1;
            }
